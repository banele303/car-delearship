...
// ─── Heatmap helpers ──────────────────────────────────────────────────────────
function buildHeatmap(rows: HourlyRow[]): { dow: number; hour: number; events: number }[][] {
  const matrix: { dow: number; hour: number; events: number }[][] = Array.from({ length: 7 }, (_, d) =>
...
function DeviceIcon({ name, className }: { name: string; className?: string }) {
  ...
  if (lower.includes('mobile') || lower.includes('phone')) return <Smartphone className={className} />;
  if (lower.includes('tablet')) return <Tablet className={className} />;
  if (lower.includes('laptop')) return <Laptop className={className} />;
  return <Monitor className={className} />;
}

// ─── Stat Card ────────────────────────────────────────────────────────────────
function StatCard({ title, value, trend, trendUp, description, icon: Icon, color, pulse = false }: {
  title: string; value: string | number; trend?: string; trendUp?: boolean;
  description?: string; icon: any; color: 'indigo'|'cyan'|'amber'|'emerald'|'purple'; pulse?: boolean;
}) {
  ...
  return (
    <Card className="p-6 rounded-3xl border-none shadow-sm dark:bg-slate-900 hover:shadow-lg transition-all group overflow-hidden relative">
      <div className="flex items-center justify-between relative z-10">
        ...
        <div className={cn("p-4 rounded-2xl relative", bg[color])}>
...
export default function TrafficAnalyticsPage() {
      ...
      <div className="flex items-center justify-center min-h-[80vh]">
        <Card className="max-w-md p-8 rounded-3xl border-none shadow-2xl dark:bg-slate-900 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-6">
            Add your PostHog Personal API Key and Project ID to your <code>.env</code> file to see live analytics.
          </p>
          <div className="space-y-4 text-left bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl mb-6">
            {[
              { n: 1, text: <>Get your <b>Personal API Key</b> from PostHog Project Settings.</> },
              { n: 2, text: <>Add <code>POSTHOG_PERSONAL_API_KEY</code> to your <code>.env</code> file.</> },
              { n: 3, text: <>Add <code>POSTHOG_PROJECT_ID</code> to your <code>.env</code> file.</> },
            ].map(({ n, text }) => (
              <div key={n} className="flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-indigo-500 text-white flex-shrink-0 flex items-center justify-center text-[10px] font-bold mt-0.5">{n}</div>
  ...
  const ErrorBanner = error ? (
    <div className="flex items-center gap-3 p-4 rounded-2xl bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-400 text-sm font-medium">
      ...
      <span>{error}</span>
      <Button size="sm" variant="ghost" className="ml-auto rounded-xl" onClick={fetchData}>Retry</Button>
    </div>
  ) : null;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            Real-time insights powered by PostHog · {lastUpdated ? `Updated ${lastUpdated.toLocaleTimeString()}` : 'Loading…'}
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          {isLoading && (
            <div className="flex items-center gap-2 text-sm text-slate-400 animate-pulse">
              <Globe className="w-4 h-4 animate-spin" /> Fetching live data…
            </div>
          )}
          <Button variant="outline" size="icon" className="rounded-xl border-slate-200 dark:border-slate-800 w-9 h-9" onClick={fetchData} disabled={isLoading}>
        ...
        {isLoading ? (
          ...
          <>
            <StatCard
              ...
              icon={Users}
              color="indigo"
              description="Unique visitors · 30 days"
            />
            <StatCard
              title="Total Pageviews"
              value={(data?.summary?.totalPageviews ?? 0).toLocaleString()}
              icon={Eye}
              color="cyan"
              description="All pageview events · 30 days"
            />
            <StatCard
              title="Pages / Visit"
              value={data?.summary?.avgPagesPerVisit ?? '—'}
              icon={LayoutGrid}
              color="amber"
              description="Avg pages per unique visitor"
            />
            <StatCard
              title="Active Now"
              value={(data?.summary?.activeNow ?? 0).toLocaleString()}
              icon={Zap}
              color="emerald"
              description="Users in last 5 minutes"
              pulse={true}
            />
          </>
        )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 p-6 rounded-3xl border-none shadow-sm dark:bg-slate-900 overflow-hidden">
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500">Daily pageviews &amp; unique visitors (last 30 days)</p>
            </div>
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trafficData} margin={{ left: -20, right: 8 }}>
                  ...
                  <Area type="monotone" dataKey="views"    name="Pageviews"       stroke="#6366f1" strokeWidth={2.5} fill="url(#gradViews)"    dot={false} activeDot={{ r: 5 }} />
                  <Area type="monotone" dataKey="visitors" name="Unique Visitors"  stroke="#22d3ee" strokeWidth={2.5} fill="url(#gradVisitors)" dot={false} activeDot={{ r: 5 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        <Card className="p-6 rounded-3xl border-none shadow-sm dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-500 mb-4">Where your visitors come from</p>

          {isLoading ? <Skeleton className="h-[240px] mb-4" /> : (
            <>
              <div className="h-[200px] w-full relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-black text-slate-900 dark:text-white">{(data?.summary?.totalPageviews ?? 0).toLocaleString()}</p>
                  </div>
                </div>
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="p-6 rounded-3xl border-none shadow-sm dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-500 mb-5">Top 10 pages by views (last 30 days)</p>
                  ...
                  {topPages.map((page) => (
                    <tr key={page.path} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="py-2.5 pr-2">
                        <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400 break-all">
                          {page.path}
                        </span>
                      </td>
      ...
      {(isLoading || countryData.length > 0) && (
        <Card className="p-6 rounded-3xl border-none shadow-sm dark:bg-slate-900">
          ...
          <p className="text-sm text-slate-500 mb-6">Top countries by pageviews (last 30 days)</p>
              ...
              {heatmap.map((row, d) => (
                <div key={d} className="flex items-center gap-1">
                  ...
                  <div className="flex flex-1 gap-0.5">
                    {row.map((cell, h) => {
                      const intensity = cell.events / maxHeat;
                      const alpha = intensity === 0 ? 0.04 : 0.1 + intensity * 0.9;
                      return (
                        <div
                          key={h}
                          className="flex-1 h-6 rounded-sm cursor-default transition-opacity hover:opacity-80"

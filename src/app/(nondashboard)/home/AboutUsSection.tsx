...
export default function AboutUsSection() {
  return (
    <section className="relative py-24 bg-white dark:bg-gray-950 overflow-hidden" aria-labelledby="about-heading">
      <div className="absolute inset-0 pointer-events-none opacity-[0.04] bg-[radial-gradient(circle_at_40%_40%,#16a34a,transparent_60%)]" />
      <div className="container mx-auto px-6 lg:px-10 relative">
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          ...
          >
            <div className="absolute -inset-4 bg-gradient-to-tr from-emerald-500/20 to-teal-500/10 rounded-3xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <motion.div
            ...
            className="space-y-6"
          >
            <h2 id="about-heading" className="text-3xl md:text-4xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-teal-600 dark:from-emerald-300 dark:to-teal-300">About Advance Auto</h2>
            ...
            <div className="grid sm:grid-cols-3 gap-5 pt-4">
              {[
                ...
                <div key={stat.label} className="rounded-xl border border-emerald-500/20 bg-emerald-50/60 dark:bg-emerald-900/10 px-4 py-5 backdrop-blur-sm">
                  <p className="text-xl font-semibold text-emerald-700 dark:text-emerald-300">{stat.value}</p>
                  <p className="text-[11px] tracking-wide uppercase font-medium text-emerald-800/70 dark:text-emerald-200/70">{stat.label}</p>
                </div>
              ))}
            </div>
            <div className="pt-4 flex flex-wrap gap-3 text-xs font-medium">
              {['Transparent Pricing','Multi‑Point Inspection','Flexible Finance','After‑Sale Support'].map(tag => (
                <span key={tag} className="px-3 py-1 rounded-full bg-emerald-600/10 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-200 border border-emerald-500/20">{tag}</span>
              ))}
            </div>
          </motion.div>
        </div>
